...
public class DeliveringExternalInboundStanzaRelayTestCase extends TestCase {
    ...
    public void testRemoteServerError() throws Exception {
        ...
        when(registry.connect(SERVER)).thenThrow(new RemoteServerNotFoundException());

        InternalServerRuntimeContext serverRuntimeContext = mock(InternalServerRuntimeContext.class);
        when(serverRuntimeContext.getServerConnectorRegistry()).thenReturn(registry);
    ...
    public void testSuccessfulRelay() throws Exception {
        ...
        when(registry.connect(SERVER)).thenAnswer(
                (Answer<XMPPServerConnector>) invocation -> {
                    // Retrieve the server name
                    Entity entity = invocation.getArgumentAt(0, Entity.class);

                    // Use PrintableInvocation to get the invocation details
                    PrintableInvocation printableInvocation = (PrintableInvocation) invocation;

                    if (printableInvocation.toString().contains(entity.toString())) {
                        return connector;
                    }
        ...
        verify(registry).connect(SERVER);
