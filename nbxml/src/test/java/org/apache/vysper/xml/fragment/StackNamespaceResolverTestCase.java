...
public class StackNamespaceResolverTestCase extends TestCase {
    ...
    public void testResolvePrefix(){
        ResolverNamespaceResolver resolver = mock(ResolverNamespaceResolver.class);
        when(resolver.resolvePrefix(anyString())).thenAnswer((InvocationOnMock invocation) -> {
            String uri = invocation.getArgumentAt(0, String.class);
            if (uri.equals(Namespaces.XML)) {
                return "xml";
            }
            return null;
        });
        String prefix1 = resolver.resolvePrefix("http://example.com");
        String prefix2 = resolver.resolvePrefix(Namespaces.XML);
        assertEquals(null, prefix1);
        assertEquals("xml", prefix2);
        verify(resolver, times(2)).resolvePrefix(anyString());
    }
}
